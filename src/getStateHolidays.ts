...
const getStateHolidays = (year: number, stateCode?: string) => {
  const holidaysByStateCode = [
    {
      state: "SP",
      date: new Date(`${String(year)}-07-09`),
      description:
    ...
    {
      state: "SE",
      date: new Date(`${String(year)}-07-08`),
      description: "Dia da Emancipação do Estado de Sergipe",
    },
    {
      state: "RR",
      date: new Date(`${String(year)}-10-05`),
      description:
        "Feriado no estado de Roraima — aniversário da elevação a estado.",
    },
    {
      state: "RS",
      date: new Date(`${String(year)}-09-20`),
      description:
        "Em 1835 foi desencadeada a Revolução Farroupilha, que manifestava o descontentamento da Província de São Pedro do Rio Grande, a mais meridional do Império, com o governo regencial do Império do Brasil.",
    },
    {
      state: "RN",
      date: new Date(`${String(year)}-10-03`),
      description:
        "Feriado dos Mártires de Cunhaú e Uruaçu, Rio Grande do Norte",
    },
    {
      state: "PI",
      date: new Date(`${String(year)}-10-19`),
      description:
        "Dia do Piauí que é Piauí é uma das 27 unidades federativas do Brasil. Localiza-se no noroeste da Região Nordeste.",
    },
    {
      state: "PR",
      date: new Date(`${String(year)}-12-19`),
      description:
    ...
    {
      state: "PA",
      date: new Date(`${String(year)}-08-15`),
      description:
        "Adesão do Pará à independência do Brasil (data magna) - Lei estadual nº 5.999/1996",
    },
    {
      state: "MG",
    ...
    {
      state: "AL",
      date: new Date(`${String(year)}-06-29`),
      description: "São Pedro - Lei estadual nº 5.509/1993",
    },
    {
      state: "AL",
      date: new Date(`${String(year)}-09-16`),
    ...
    {
      state: "CE",
      date: new Date(`${String(year)}-03-19`),
      description:
        "Dia de São José (Padroeiro do Ceará) - Lei Federal nº 9.093/1995",
    },
    {
      state: "CE",
      date: new Date(`${String(year)}-03-25`),
      description:
    ...
    {
      state: "DF",
      date: new Date(`${String(year)}-11-30`),
      description: "Dia do evangélico - Lei distrital nº 963/1995",
    },
    {
      state: "AM",
      date: new Date(`${String(year)}-09-05`),
      description:
    ...
    {
      state: "AC",
      date: new Date(`${String(year)}-11-17`),
  ...
  return holidaysByStateCode.filter(
    holiday => holiday.state === stateCode.toUpperCase(),
  )
}

export const isStateHoliday = (date: Date, stateCode?: string) => {
  const stateHolidays = getStateHolidays(date.getFullYear(), stateCode)

  return stateHolidays.some(holiday => isSameDay(holiday.date, date))
}
